...
class BusinessLogic {
    ...
    public function _CallBeneficiaryRegistrationAPI($mobile_no, $fname, $lname, $account_no, $ifsc_code, $sender_profile_id) {
        ...
        if (!$api_result['isError']) {
            ...
            if ($api_result['array_data']['error_code'] == '0') {
                $beneficiary_id = $api_result['array_data']['sender_beneficiaries']['beneficiary_id']; //store beneficiary_id in your db it is used for fund transfer api
    ...
    public function _CallTransactionStatusAPI($transaction_id) {
        ...
        if (!$api_result['isError']) {
            ...
            if ($api_result['array_data']['error_code'] == '0') {
                ...
                if ($api_result['array_data']['transaction_details']['status'] == 'RefundPending') {
                    //Call Refund OTP API, After Call Refund API to Claim Refund
                    echo $api_result['array_data']['message'];
                }
