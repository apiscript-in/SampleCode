...
class BusinessLogic {
    ...
    public function _CallFundTransferAPI($mobile_no,$sender_profile_id,$beneficiary_id,$amount,$transfer_mode='IMPS') {
        $api = new APICall();
        $api_result = $api->CallFundTransferAPI($$mobile_no,$sender_profile_id,$beneficiary_id,$amount);
        if(!$api_result['isError']){
            if($api_result['array_data']['error_code'] == '1'){
